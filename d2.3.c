...
void shortest(GraphType *G, int P[][MAX], int D[][MAX]) //输出最短路径、以及需要经过的景点
{
	...
	if (m < n)
	{
		printf("%s与%s之间的最短路径长度为%d\n", G->vertexs[m].name, G->vertexs[n].name, D[m][n]);//输出最短路径
		printf("path: %s", G->vertexs[n].name);            //输出源点
		while (P[m][n] != 0)                               //经过该节点
		{
			printf("<--%s", G->vertexs[P[m][n]].name);     //输出路径的下一个节点
			if (m < n)
				n = P[m][n];
			else
				m = P[n][m];
...
void floyd(GraphType *G)   // 核心算法          弗洛伊德 算法时间复杂度:O(n3)
{
	int D[MAX][MAX];       //D[i][j]记录最短路径权值
	int P[MAX][MAX];       // P[i][j]为i到j的当前最短路径
	int i, j, k;
	for (j = 1; j <= G->vexnum; j++)
	{
		for (k = 1; k <= G->vexnum; k++)
		{
			D[j][k] = G->arcs[j][k].adj;       //初始化两点之间的权值D
			P[j][k] = 0;                       //初始化P   未开始查找，所以中间节点初始化为0
		}
	}
	for (i = 1; i <= G->vexnum; i++)           //向i与j之间k次加入中间节点
	{
		for (j = 1; j <= G->vexnum; j++)
		{
			for (k = 1; k <= G->vexnum; k++)
			{
				if (D[j][k] > D[j][i] + D[i][k])      //判断路径分开还是直达     判断最短路径
				{
					D[j][k] = D[j][i] + D[i][k];
					P[j][k] = i;
					P[k][j] = i;  //记录经过的路径
...
{
	int num0, num; //num0 景点编号 ，num 更改信息编号
	fp = fopen("D:\\file.txt", "at+");  //打开文件，a指写入处理的时候按照文件已有内容写入，t指打开文本类型为文本文件，+指对文件读取+写入
	printf("请输入所修改景点编号:");
	scanf("%d", &num0);
